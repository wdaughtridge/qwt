...
impl QwtBidirectional {
    ...
    pub async fn start(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        while let Ok(Some(data)) = self.bidi.receive().await {
            debug!("Got WebTransport {} bytes of bidirectional data", data.len());
...
impl QwtClient {
    ...
    pub async fn start(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        if let Some(rx) = self.conn.accept_receive_stream().await? {
            ...
            tokio::spawn(async move {
                QwtClient::handle_receive_control_stream(rx, tx)
                    .await
        ...
        if let Ok(tx) = self.conn.open_send_stream().await {
            ...
            tokio::spawn(async move {
                QwtClient::handle_send_control_stream(tx, rx)
                    .await
                    .expect("server control")
            });
        }
        ...
        if let Some(bidi) = self.conn.accept_bidirectional_stream().await? {
            ...
            tokio::spawn(async move {
                QwtClient::handle_bidirectional_request_stream(bidi)
                    .await
                    .expect("bidi request")
            });
        }
        ...
        loop {
            tokio::select! {
                Ok(Some(bidi)) = self.conn.accept_bidirectional_stream() => {
                    tokio::spawn(async move {
                        QwtClient::handle_bidirectional_stream(bidi).await.expect("bidi stream");
                    });
                }
            ...
            tokio::select! {
                Ok(Some(rx)) = self.conn.accept_receive_stream() => {
                    tokio::spawn(async move {
                        QwtClient::handle_receive_stream(rx).await.expect("unidi stream");
                    });
                }
    ...
    async fn handle_bidirectional_request_stream(
        ...
        while let Ok(Some(data)) = bidi.receive().await {
            ...
            let frm = data.get_bytes(len as usize)?;
            ...
            match typ {
                0x01 => {
                    ...
                    let hdrs = dec.decode(frm.buf(), 1024)?;
                    ...
                    let len = enc.encode(&hdrs, &mut buf[..])?;
    ...
    async fn handle_receive_control_stream(
        ...
        let stream_type = rx.read_u8().await?;
        ...
        while let Ok(Some(data)) = rx.receive().await {
            ...
            let typ = data.get_varint().unwrap();
            let len = data.get_varint().unwrap();
            let mut frm = data.get_bytes(len as usize)?;
            ...
            match typ {
                0x04 => {
                    ...
                    let len = {
                        ...
                        if "draft-02" == ver {
                            frm.put_varint(SETTINGS_ENABLE_WEBTRANSPORT)?;
                            frm.put_varint(1)?;
                        } else {
                            frm.put_varint(SETTINGS_WEBTRANSPORT_MAX_SESSIONS)?;
                            frm.put_varint(1024)?;
                        }

                        // Required in all WebTransport drafts
                        frm.put_varint(SETTINGS_H3_DATAGRAM)?;
                        frm.put_varint(1)?;
    ...
    async fn handle_receive_stream(mut rx: ReceiveStream) -> Result<(), Box<dyn Error>> {
